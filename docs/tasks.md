...
### T11. 廃止届出オープンデータの取得設定追加
- **概要**: 川崎市薬務課ページ（#opendata_dataset_6）の CSV 資料をダウンロードする設定を追加し、既存のオープンデータ取得処理で扱えるようにする。
- **Acceptance Criteria**
  - 新しい OpenDataPage 設定で 3 つの CSV（薬局・店舗販売業・卸売販売業の廃止届出）を絶対 URL で管理できる。
  - `download_opendata_page` を通じて 3 ファイルすべてがページ専用ディレクトリに保存されることをテストで確認できる。
- **DoD**: グローバル DoD。設定は静的に定義し、テストでカバレッジを確保すること。
- **Status: DONE**
  - `PHARMACY_CLOSURES_PAGE` を追加し、ページアンカー/リソース URL を絶対パス化。
  - 既存のオープンデータダウンロードテストをパラメータ化し、新ページの 3 リソース保存とファイル名検証を網羅。
  - `OPEN_DATA_PAGES` でオープンデータページ集合を共有し、最新の main 基点で再検証。
