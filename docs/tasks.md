...
### T11. データセットカタログのドキュメント整備
- **概要**: オープンデータカタログのカテゴリリンクとデータセット URL を一覧化し、`configs/` に登録済みのものを消込できるドキュメントを追加する。
- **Acceptance Criteria**
  - カテゴリリンクおよびデータセット一覧を追記できるテンプレートが docs 配下に存在する。
  - 既存の config/ローダーで取り込み準備ができているデータセットが明示的に分かる。
  - MkDocs ナビゲーションから新ドキュメントへ遷移できる。
- **DoD**: グローバル DoD。既存の設定済みデータセットが一覧表に反映されていること。
- **Status: DONE**
  - docs/datasets/catalog.md を追加し、カテゴリリンク控え・データセット表・消込済みリストのフォーマットを用意。
