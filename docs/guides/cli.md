...
The Command Line Interface (CLI) provides an interactive way to work with Kawasaki ETL.
...
uv run python -m kawasaki_etl.main --dotenv custom.env
...
uv run python -m kawasaki_etl.main --dotenv /path/to/config.env

# Short form
uv run python -m kawasaki_etl.main -e config.env
```
...
# Change log format
LOG_FORMAT=plain uv run python -m kawasaki_etl.main
```
...
```python
# src/kawasaki_etl/interfaces/cli.py
...
```bash
LOG_LEVEL=DEBUG LOG_FORMAT=console uv run python -m kawasaki_etl.main
```
...
```bash
echo "hello" | uv run python -m kawasaki_etl.main
```
...
```bash
alias ci='uv run python -m kawasaki_etl.main'
```
