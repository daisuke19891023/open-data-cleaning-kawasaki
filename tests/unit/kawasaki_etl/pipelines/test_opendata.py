...
def test_download_opendata_page(monkeypatch: pytest.MonkeyPatch, tmp_path: Path) -> None:
    ...
    assert {dest.parent for _, dest in called} == {tmp_path / PHARMACY_PERMITS_PAGE.storage_dirname}

    expected_filenames = {resource.filename for resource in PHARMACY_PERMITS_PAGE.resources}
